#!/usr/bin/env node
/**
 * PDX Food Week Scraper — structured extraction from EverOut Wiener Week dish pages.
 *
 * Usage:
 *   node scrapers/scrape_wieners.js [--force]
 *
 * Output: data/wienerweek2026.js (overwritten)
 */
...
const WEEK_URL    = 'https://everout.com/portland/events/the-portland-mercurys-wiener-week-2026/e222740/';
const PARENT_EID  = 'e222740'; // exclude the parent wiener-week event itself
...
function parseDishPage(html, url) {
  ...
  // Emoji & spicy heuristic for wieners
...
async function getDishLinks() {
  console.log('Fetching Wiener Week index…');
...
async function main() {
  ...
  const outPath = path.join(outDir, 'wienerweek2026.js');
  ...
  const header = `// Portland Mercury's Wiener Week 2026 — scraped ${new Date().toISOString().slice(0, 10)}
...
(function() {
  ...
  updateMetaTotalLocations('wiener-2026', entries.length, path.resolve(__dirname, '../js/meta.js'));
